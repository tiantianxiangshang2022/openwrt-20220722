...
sul = ful:section(SimpleSection, "", translate("Upload file"))
...
dir = "/tmp/upload/"
if luci.http.formvalue("uldir") then
	dir = luci.http.formvalue("uldir")
end
...
http.setfilehandler(
	function(meta, chunk, eof)
		if not fd then
			...
			if fs.isfile(dir .. meta.file) then
				um.value = translate("File is already exist:") .. ' "' .. dir .. meta.file .. '"'
				return
			end
...
btnrm.write = function(self, section)
	local fp, v
	fp = "/tmp/upload/" .. luci.fs.basename(inits[section].name)
	if fs.isdirectory(fp) then
		v = nixio.fs.remover(fp)
	else
		v = luci.fs.unlink(fp)
	end
